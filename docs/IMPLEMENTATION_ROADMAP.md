...
- **⚠️ DECISION NEEDED — walkability blocked on CONSTRAINT-001:** walkability emits a 0–100 composite `score` + graded category (Walker's Paradise/…) — a numerical quality rating the constraint forbids; the schema has no `score` field. Options: (a) **surface only the underlying destination counts** (grocery/transit/park within walking distance) as factual measures — recommended; (b) omit walkability from the contract; (c) treat Walk-Score-style proxies as an exempt external metric. **Get Nathan's call before building walkability's contract.**
- **▶▶ DO NEXT — continue non-located chapters. 9 of 14 done** (utilities, community, health, schools, safety, reachability, recreation, access, property). Clean next candidates: **environment** (FEMA flood zone + EPA air — factual/categorical, good `comparison{}` use), **growth**, **garden** (richer qualitative data). Multi-source ones (climate) last. Walkability gated on the decision above. Per-module `contract.js` + `safeBuild` + Zod + per-address snapshots incl. Jeffersonville; derive `tone` from existing semantic signals (drop colors/labels — CONSTRAINT-001/008), surface `comparison{basis, referenceValue, direction, deltaPct}` for state/national-relative data (CONSTRAINT-002: national median only for income). See "Contract rollout pattern" below.
