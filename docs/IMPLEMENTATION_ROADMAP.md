...
- **Most recent (main):** FR-078 (Headless Report Contract — utilities pilot: Zod-validated, presentation-free report contract + `GET /api/report.json`; first step to a totally headless backend with the frontend built separately) — **merged (PR #48, squash).** Full suite 1,686 / 89. See the session-5 hand-off above + `feature-requests/FR-078-headless-report-contract/`. *(Prior: FR-077 fan-out #47; FR-076 utilities pilot #46.)*
