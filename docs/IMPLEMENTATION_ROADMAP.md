...
- **Tests: 1,493 across 79 suites** (on `main`)
...
- **Most recent (main):** FR-065 (source-chain resilience primitive + NOAA→Open-Meteo climate-normals fallback) — **merged (PR #30, squash).** First slice of Track A1; see Track A item 1 + `feature-requests/FR-065-source-chain-resilience/`. Full suite 1,493 / 79.
...
**Hardening Stage 0, A2 (FR-063), the B1/B2 attempt, and A1's first slice (FR-065) are now behind us.** **B1/B2 remain DEFERRED on human-in-the-loop blockers** (2026-06-16: FR-062 needs the FCC BDC token; NREL is a deploy-time live-verify, still DNS-unreachable from dev) — see Track B items 1–2. **A1 is in progress:** FR-065 (PR #30) shipped the reusable `sourceChain` primitive + the NOAA→Open-Meteo climate-normals slice. Revised order: **Stage 0 → A2 (FR-063) → ~~B1/B2~~ (deferred) → A1 [primitive + NOAA slice done; more slices remain] → Hardening Stage 1** (state externalization, before any B2B contract). **Next up: continue A1** — each remaining slice is a cheap reuse of `sourceChain`. Suggested order by value: **Google-POI cost-resilience fallback** (OSM for *non-safety* POIs only — directly de-risks the Google cost/quota exposure), then USDA soil, USGS elevation, Census vintage. The recommended near-term polish on FR-065 itself: **cell-cache the modeled-normals result** (the Open-Meteo per-minute rate limit makes this worthwhile for bulk/B2B). Track B breadth items follow one FR at a time as upside.
