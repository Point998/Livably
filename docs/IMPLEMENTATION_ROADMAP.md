...
- **▶▶ DO NEXT — non-located chapters. 8 of 14 chapters done** (utilities, community, health, schools, safety, reachability, recreation, access). **The `place{}` located-facility pattern is exhausted** — every remaining chapter (climate, property, growth, garden, environment, walkability, costs…) is **non-located with richer/varied data** (multi-field measures, comparisons to state/national, qualitative bands). Each needs **bespoke finding design from its `logic.js` output** — do NOT copy the located-facility template. Recommended next: **property** or **walkability** (cleaner single-source data) before the multi-source ones (climate/environment). Still per-module `contract.js` + `safeBuild` + Zod conformance + per-address snapshots incl. Jeffersonville (see "Contract rollout pattern" below); derive `tone` from existing semantic signals, surface `comparison{basis, referenceValue, direction, deltaPct}` for national/state-relative data (CONSTRAINT-002: national median only for income).
