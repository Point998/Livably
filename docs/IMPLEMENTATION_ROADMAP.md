...
- **Tests: 1,539 across 81 suites** (on `main`)
...
- **State:** `main` clean @ `13df115`, **1,539 tests / 81 suites** green, CI on every push, working tree clean. No open PRs awaiting action.
- **Shipped (this track):** FR-065 (PR #30 — reusable `sourceChain` resilience primitive + NOAA→Open-Meteo climate-normals fallback; honest-provenance principle), FR-066 (PR #32 — Google-POI→OSM cost-resilience fallback for Reachability + shared `overpass.js`/`osmPlaces.js`; fixed a latent Overpass-406 bug that was also silently degrading Sensory's OSM features), **FR-067 (PR #35 — Walkability Google→OSM fallback; one Overpass union call via new opt-in `searchOSMPOIs({withTags})` + client-side `categorizeOSMWalkPOI`; also fixed a hidden observability gap where the Google fetcher swallowed total failure to `score:0`, masking outages green in the monitor).** B1/B2 re-confirmed deferred (PR #29); roadmap kept current (#31, #33).
- **NEXT UP (no blockers, highest value): continue A1 — extend the OSM fallback to the remaining single-source Google modules**, each a cheap reuse of `searchOSMPOIs` (same pattern as FR-066/067; full 4-phase workflow; none are safety tier): ~~(1) Walkability~~ ✅ FR-067 → **(2) Recreation** (park/coffee/library/rec/post office — next) → **(3) Sensory airports** (OSM tag `aeroway=aerodrome`, not shop/amenity) → **(4) Growth commercial**. Then non-Google single sources: USDA soil, USGS elevation, Census vintage. *Note: the `withTags` + client-side categorization pattern (FR-067) is reusable for any multi-category OSM module like Recreation — consider promoting a shared "categorized OSM search" helper once there's a second caller.*
- **Watch-items:** observability debt (FR-067 paid down one slice — the swallow-to-empty walkability outage mask — but no dashboard yet, Hardening Stage 2); single-instance state ceiling (Stage 1, the B2B gate); carryover — cell-cache the FR-065 modeled-normals result (Open-Meteo per-minute rate limit).
...
- **Most recent (main):** FR-067 (Walkability Google→OSM cost-resilience fallback; opt-in `searchOSMPOIs({withTags})` + `categorizeOSMWalkPOI`; outage-masking observability fix) — **merged (PR #35, squash).** Third slice of Track A1; see Track A item 1 + `feature-requests/FR-067-walkability-osm-fallback/`. Full suite 1,539 / 81.
