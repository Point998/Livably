...
- **▶▶ DO NEXT — continue the rollout. 6 of 14 chapters done** (utilities, community, health, schools, safety, reachability). Remaining located-facility chapters: **recreation** (park/coffee/library/rec/post — reuses `place{}`, several have OSM straight-line fallbacks like reachability) and **access** (highway on-ramp). Then the non-located chapters (climate, property, growth, garden, etc.). Same proven pattern (see "Contract rollout pattern" below).
