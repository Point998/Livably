...
- **▶▶ DO NEXT — continue non-located chapters. 12 of 14 done** (utilities, community, health, schools, safety, reachability, recreation, access, property, walkability, environment, growth). Remaining: **garden** (planting zone / frost / flora — mostly factual/categorical), **costs** (tax/insurance/utilities estimates from `propertyData` + the cost calculator — watch for the CONSTRAINT-001 "score" trap and CONSTRAINT-002 income framing: national median only), multi-source **climate** (heat/seismic/precip — apply the FR-090 external-index principle). **Deferred follow-on:** sensory **ambiance** items (airports / rail / light pollution / Bortle) — a small additive FR to `sensory/contract.js`. Per-module `contract.js` + `safeBuild` + Zod + per-address snapshots incl. Jeffersonville; derive `tone` from existing semantic signals (drop colors/labels); **external standard indices = factual measures (FR-090); Livably composites → underlying facts (walkability FR-089); value-neutral context → neutral tone (growth FR-091).** See "Contract rollout pattern" below.
