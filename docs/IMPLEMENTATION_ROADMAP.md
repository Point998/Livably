...
- **▶▶ DO NEXT — continue non-located chapters. 11 of 14 done** (utilities, community, health, schools, safety, reachability, recreation, access, property, walkability, environment). Remaining: **growth**, **garden**, **costs**, multi-source **climate** (heat/seismic — apply the external-index principle from FR-090). **Deferred follow-on:** sensory **ambiance** items (airports / rail / light pollution / Bortle) — left out of FR-090, a small additive FR to the same `sensory/contract.js`. Per-module `contract.js` + `safeBuild` + Zod + per-address snapshots incl. Jeffersonville; derive `tone` from existing semantic signals (drop colors/labels — CONSTRAINT-001/008); **external standard indices = factual measures (FR-090 principle); Livably-computed composites must be reduced to underlying facts (walkability FR-089).** Watch for more score/grade traps in growth/costs/climate. See "Contract rollout pattern" below.
