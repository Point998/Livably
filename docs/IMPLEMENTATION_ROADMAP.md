...
- **▶▶ DO NEXT — continue the rollout. 7 of 14 chapters done** (utilities, community, health, schools, safety, reachability, recreation). Last located-facility chapter: **access** (highway on-ramp — `findNearestHighwayOnRamp` in `src/modules/access/data.js`; geocoding strategy per CONSTRAINT-005/PM-002, single record, reuses `place{}`). Then non-located chapters (climate, property, growth, garden, environment, etc.) — richer/varied data shapes, so each needs its own finding design (no `place{}` reuse). Same proven pattern (see "Contract rollout pattern" below).
