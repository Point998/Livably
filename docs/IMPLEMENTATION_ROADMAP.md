...
### ▶ Session hand-off — 2026-06-23 (session 6 — **FR-080 Health + FR-081 Schools contracts**, read first)
*Next session starts here. This session shipped **FR-080** (health, contract's first schema evolution) and **FR-081** (schools, reused the new `place{}` primitive), and discovered + documented a cross-state bug (**PM-005 / FR-082**).*
...
- **⚠️ Discovered — PM-005 / FR-082 (cross-state school, CONSTRAINT-006 gap, NOT yet fixed):** `getSchoolRatings()` (the chapter data path) has **no `checkCrossState` filter**, unlike `findNearestSchool` (PM-001). A border address (Jeffersonville IN) can surface a cross-state KY school in the schools chapter today. The chapter data shape has no `location`/`state`, so the FR-081 contract **cannot** flag it — the fix must live in `getSchoolRatings` at fetch time. Documented in `docs/postmortems/PM-005-cross-state-school-chapter-path.md`; fix spec in `feature-requests/FR-082-schools-crossstate-filter/`. **Reinforces CONSTRAINT-014** (apply coherence to ALL searches of a class, not just the function named in a bug). Follow-on audit: re-check hospital/pharmacy/grocery multi-path searches for the same gap.
