...
- **▶▶ DO NEXT — continue the rollout. 5 of 14 chapters done** (utilities, community, health, schools, safety). Next located-facility chapter: **reachability** (grocery/pharmacy/gas + civic — reuses `place{}`; note pharmacy now carries `crossStateWarning`/`crossStateNote` from FR-083, which the contract should surface as `tone:caution` + note, mirroring health). Same proven pattern (see "Contract rollout pattern" below).
