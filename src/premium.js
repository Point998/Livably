...
function buildUpgradeCTAHTML(reportId) {
  const features = [
    { icon: '🏫', label: 'Schools & Education', desc: 'Assigned school ratings, trajectory, and district context' },
    { icon: '🚑', label: 'Health & Safety', desc: 'Fire station ISO rating, EMS response, traffic safety' },
    { icon: '🌊', label: 'Climate & Weather Risks', desc: 'Flood zone (parcel-level), tornado risk, insurance implications' },
    { icon: '🔬', label: 'Sensory & Environmental', desc: 'Air quality, water violations, radon zone, airport noise' },
    { icon: '🏗️', label: 'Growth & Development', desc: 'Confirmed funded projects, named developments with status' },
    { icon: '🏠', label: 'Property Intelligence', desc: 'Construction era, soil drainage, broadband, permit guidance' },
    { icon: '🚶', label: 'Walkability', desc: 'Pedestrian access to groceries, transit, dining, pharmacy' },
    { icon: '📊', label: 'Property & Market Data', desc: 'Tax history context, carrying costs, market indicators' },
  ];

  const featureRows = features.map((f) =>
    `<div class="upgrade-feature">
      <span class="upgrade-feature-icon">${f.icon}</span>
      <div>
        <div class="upgrade-feature-label">${esc(f.label)}</div>
        <div class="upgrade-feature-desc">${esc(f.desc)}</div>
      </div>
    </div>`,
  ).join('');

  const stripeScript = reportId ? `
    <script>
      document.getElementById('unlockBtn').addEventListener('click', async function () {
        ...
        try {
          var resp = await fetch('/create-checkout-session', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ reportId: '${esc(reportId)}' }),
          });
          ...
          if (data.url) {
            ...
            btn.disabled = false;
            btn.textContent = 'Unlock Premium — $24.99';
            alert(data.error);
          }
        } catch (e) {
          btn.disabled = false;
  ...
  return `
  <div class="upgrade-cta chapter-card">
    <div class="upgrade-cta-header">
      <div class="chapter-label">Livably Premium</div>
      <div class="upgrade-cta-title">Go deeper on the things that matter most.</div>
      <p class="upgrade-cta-intro">Your standard report covers daily essentials. Premium unlocks eight additional chapters covering health infrastructure, climate risk, property history, and everything that takes weeks to discover on your own.</p>
    </div>
    <div class="upgrade-features">${featureRows}</div>
    <div class="upgrade-cta-action">
      <div class="upgrade-price-block">
        <span class="upgrade-price">$24.99</span>
        <span class="upgrade-price-label">one-time · this report only</span>
      </div>
      <button id="unlockBtn" class="upgrade-btn">Unlock Premium — $24.99</button>
      <p class="upgrade-note">Secure payment via Stripe · Instant access · No subscription</p>
    </div>
  </div>${stripeScript}`;
}

// All premium sections in display order.
// isPremium: true = show content; false = show upgrade CTA.
function buildPremiumSectionsHTML(premium, isPremium, reportId) {
  if (!premium) return '';
  if (!isPremium) return buildUpgradeCTAHTML(reportId || null);
  // Chapter order per CLAUDE.md standard tier spec:
  // Ch 3: Schools & Education
  // Ch 1: Health & Safety (via Crime/Emergency)
